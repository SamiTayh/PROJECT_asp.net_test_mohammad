...
{
    public class TravelssController : BaseController
    {
        private readonly IRepositoryBase<Travel> _repo;
        ...
        public TravelssController(IRepositoryBase<Travel> repo,
        ...
        {
            List<Travel> lists = await _repo.GetList(includes: new List<string> { "Gender", "ListBooks" });
            List<TravelVM> viewmodel = _mapper.Map<List<Travel>, List<TravelVM>>(lists);
        ...
        public async Task<IActionResult> Form(int ? id)
        {
            ...
            {
                Travel entity = await _repo.GetOne(z => z.Id == id);
                vm = _mapper.Map<Travel, TravelVM>(entity);
                return PartialView(vm);
            }

        }
        [HttpPost]
        public async Task<IActionResult> Form(TravelVM insert)
        {
            Travel model;
            model = _mapper.Map<TravelVM, Travel>(insert);
        ...
        [HttpPost]
        public async Task<IActionResult> Delete(Travel del )
        {
            bool done = false; 
