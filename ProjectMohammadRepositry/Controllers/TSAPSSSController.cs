...
{
    public class TSAPSSSController : BaseController
    {
        private readonly IRepositoryBase<TSAP> _repo;
        ...
        public TSAPSSSController(IRepositoryBase<TSAP>repo,
        ...
        {
            List<TSAP> lists = await _repo.GetList(includes: new List<string> { "Gender", "ListBooks", "Author" });
            List<TSAPVM> viewmodel = _mapper.Map<List<TSAP>, List<TSAPVM>>(lists);
        ...
        public async Task<IActionResult> Form(int ? id)
        {
            ...
            {
                TSAP entity = await _repo.GetOne(z => z.Id == id);
                vm = _mapper.Map<TSAP, TSAPVM>(entity);
        ...
        [HttpPost]
        public async Task<IActionResult>Form(TSAPVM insert)
        {
            TSAP model;
            model = _mapper.Map<TSAPVM, TSAP>(insert);
        ...
        [HttpPost]
        public async Task<IActionResult> Delete(TSAP del)
        {
            bool done = false;
