...
{
    public class DepartmentssController : BaseController
    {
        private readonly IRepositoryBase<Department> _repo;
        private readonly IRepositoryBase<Author> _listAuthor;
        private IMapper _mapper;
        private IHostingEnvironment Environment;
        private readonly ISessionConfigService _sessionService;
        public DepartmentssController(IRepositoryBase<Department> repo,
        ...
        {
            List<Department> lists = await _repo.GetList(includes:new List<string> { "Author" });
            List<DepartmentVM> viewmodel = _mapper.Map<List<Department>, List<DepartmentVM>>(lists);
        ...
        public async Task<IActionResult> Form(int?id)
        {
            ...
            {
                Department entity = await _repo.GetOne(z => z.Id == id);
                vm = _mapper.Map<Department, DepartmentVM>(entity);
        ...
        [HttpPost]
        public async Task<IActionResult> Form(DepartmentVM insert)
        {
            Department model;
            model = _mapper.Map<DepartmentVM, Department>(insert);
        ...
        [HttpPost]
        public async Task<IActionResult> Delete(Department del)
        {
            bool done = false;
